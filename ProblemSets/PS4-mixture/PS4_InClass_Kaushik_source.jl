...
function practice_monte_carlo()
    ...
    for D in [1_000, 1_000_000] # numbers in julia can have underscores
        println("\nWith D = $D draws:")
...
function mixed_logit_quad(theta, X, Z, y, R)
    ...
    # Hint: Use mu_gamma + sigma_gamma * nodes[r] to get gamma_r
    # and weight by weights[r] * φ(nodes[r])
    # You can use 
    nodes, weights = lgwt(7, mu_gamma-5*sigma_gamma, mu_gamma+5*sigma_gamma)  # example nodes and weights
    ...
    # loop through grid to do summation via loop
...
function mixed_logit_mc(theta, X, Z, y, D)
    ...
    # Similar to quadrature but with random draws instead of nodes/weight
